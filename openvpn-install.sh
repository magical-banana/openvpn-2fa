...
function newClient() {
	...
	if [[ $CLIENTEXISTS == '1' ]]; then
		...
		if [ ! -d "/opt/openvpn/clients" ]; then
			mkdir -p /opt/openvpn/clients
		fi
		if [ ! -d "/opt/openvpn/google-auth" ]; then
			mkdir -p /opt/openvpn/google-auth
		fi
		echo "Add user $CLIENT to vpnserver"
		useradd -m -d "/opt/openvpn/clients/$CLIENT" -s /bin/nologin $CLIENT
		CLIENT_PASSWORD="$(openssl rand -base64 20)"
		echo "$CLIENT:$CLIENT_PASSWORD" | sudo chpasswd
		echo "Password for user $CLIENT is: $CLIENT_PASSWORD"
		echo "Generating QR code for user $CLIENT"
	...
	{
		echo
		# Enable username, password and OTP at client side
		# echo "auth-nocache"
		echo "remote-cert-tls server"
		echo "reneg-sec 0"
		echo "auth-user-pass"
		echo 'static-challenge "Enter Authenticator Code" 1'
		echo
		echo "<ca>"
...
function revokeClient() {
	...
	echo "Deleting user $CLIENT"
	userdel -rf $CLIENT
	echo "Deleting user $CLIENT 2fa profile"
	rm -f /opt/openvpn/google-auth/$CLIENT
	echo ""
